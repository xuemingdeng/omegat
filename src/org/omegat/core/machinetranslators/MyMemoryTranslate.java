...
public class MyMemoryTranslate extends BaseTranslate {
    ...
    protected String translate(Language sLang, Language tLang, String text) throws Exception {
        ...
        // Adjust DTD location and bug in entity encoding; 
        // TODO: Use local DTD
        // TODO: Remove second line as soon as the bug is fixed by MyMemory;  
        tmxResponse = tmxResponse.replace("<!DOCTYPE tmx SYSTEM \"tmx11.dtd\">", "");
        tmxResponse = tmxResponse.replace("&", "&amp;").replace("&amp;amp;", "&amp;");
	...
	private String getBestTranslation(Language sLang, Language tLang,
        ...
        String sourceSeg = "";
        String targetSeg = "";
        XPathExpression sExpr;
        XPathExpression tExpr;
        ...
        // Set up xpath queries
    	sExpr = xpath.compile(sourceSegQueryString);
     	tExpr = xpath.compile(targetSegQueryString);
                
        // Loop over TUs to get best matching source segment and its translation
		for (int i = 0; i < allTUs.getLength(); i++) {
        	tu = allTUs.item(i);
        	
        	sourceSeg = ((NodeList) sExpr.evaluate(tu, XPathConstants.NODESET)).item(0).getNodeValue();       	
            targetSeg = ((NodeList) tExpr.evaluate(tu, XPathConstants.NODESET)).item(0).getNodeValue();
        	
           	dist = EditDistance.editDistance(text, sourceSeg); 
